...
screenshot_alt: "@sebastienrousseau/browserslist-config architecture and rule enforcement."
footer_note: "@sebastienrousseau/browserslist-config is part of the @sebastienrousseau/config suite. Published under Apache-2.0 OR MIT."
copyright: "© 2026 Sebastien Rousseau. Licensed under Apache-2.0 OR MIT."
translation_key: "home"
title: "@sebastienrousseau/browserslist-config — Shareable Browserslist target configuration rules for modern browser compatibility and bundle optimization."
description: "Shareable Browserslist target configuration rules for modern browser compatibility and bundle optimization."
keywords: "browserslist-config, browserslist, browser support, babel, autoprefixer, caniuse, javascript, developer tools, sebastien rousseau, standard"
hero_tag: "v0.0.3"
hero_tagline: "Modern browser compatibility and target matrix configuration"
headline: "@sebastienrousseau/browserslist-config"
lead: "Shareable Browserslist target configuration rules for modern browser compatibility and bundle optimization."
cta_primary: "Install @sebastienrousseau/browserslist-config"
...
snippet_caption: "Install @sebastienrousseau/browserslist-config"
snippet_code: "npm install --save-dev @sebastienrousseau/browserslist-config"
...
## What is @sebastienrousseau/browserslist-config?

**@sebastienrousseau/browserslist-config** is an enterprise-grade, shareable configuration for [Browserslist](https://github.com/sebastienrousseau/browserslist-config)—the industry-standard browser targeting specification used across Babel, Autoprefixer, PostCSS, and Vite. Designed to deliver vetted, deterministic defaults, this package eliminates manual setup boilerplate and establishes rigorous code quality standards across modern software development pipelines.

By adopting this configuration, engineering teams ensure unified consistency across local development, continuous integration, and automated deployments. The package adheres strictly to the OpenSSF Best Practices badge program (ID: 14505) and achieves top-tier security ratings via Scorecard audits.
...
## Why Developers Choose @sebastienrousseau/browserslist-config

Modern JavaScript and TypeScript repositories frequently suffer from configuration drift, fragmented standards, and maintenance overhead. **@sebastienrousseau/browserslist-config** directly solves these challenges:
...
Integrating `@sebastienrousseau/browserslist-config` into any new or existing repository requires only three simple steps.
...
### Step 2: Configure `.browserslistrc`

Create or update your `.browserslistrc` file at the root of your project to extend the shared configuration:

```text
extends @sebastienrousseau/browserslist-config
```

Learn how to customize specific rules and override settings in the [Usage Guide](usage/).

### Step 3: Run Browserslist in Your Workflow

Execute Browserslist directly or configure standard quality scripts in your `package.json`:

```bash
npx browserslist
```

Display your evaluated browser query and global coverage percentage.
...
### Why use a shared Browserslist configuration?
A centralized Browserslist configuration ensures that build tools like Babel, SWC, Autoprefixer, and Vite target identical browser versions, preventing polyfill duplication and unexpected syntax regressions.

### Can I override individual rules in my project?
Yes. Every configuration allows local overrides directly within your `.browserslistrc` file or inline settings. Detailed override recipes are documented in the [Usage Guide](usage/).

### Is @sebastienrousseau/browserslist-config safe for production CI/CD?
