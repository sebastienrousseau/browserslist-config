# `@sebastienrousseau/browserslist-config` Features & Capabilities

An exhaustive overview covering **100% of the features, rules, and capabilities** provided by `@sebastienrousseau/browserslist-config`.
...
| :--- | :--- | :--- | :--- |
| **Global Usage Threshold** | Core | Active | Requires browser share > 0.5% worldwide |
| **Recent Engine Support** | Core | Active | Targets last 2 major versions of all evergreen browsers |
| **Enterprise Support** | Core | Active | Maintains compatibility with Firefox Extended Support Release (ESR) |
| **Dead Browser Pruning** | Core | Active | Explicitly excludes unsupported/abandoned browser versions (`not dead`) |
| **Proxy Browser Pruning** | Core | Active | Explicitly excludes legacy proxy browsers (`not OperaMini all`) |
